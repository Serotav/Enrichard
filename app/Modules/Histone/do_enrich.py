...
def main():
    parser = argparse.ArgumentParser(description="Perform histone enrichment analysis.")
    ...
    # --- 1. Load Data ---
    try:
        background_path = next((APP_DIR / "histone_annotated").glob(f"{args.background_name}.hg*annotated.parquet"))
    ...
    heatmap_df = corrected_results_df.with_columns(
        ...
        pl.col("Trait").str.split_exact("_", 1).struct.rename_fields(["Cell_Type", "Enzyme"])
    ).unnest("Trait")
    ...
    try:
        ...
        # Get the enzyme columns before the join
        enzyme_columns = pivoted_df.columns[1:]
        ...
        final_df = pivoted_df.join(
            ...
            *enzyme_columns # Keep all the enzyme columns
        ).fill_null("Unknown")

        final_df.write_csv(destination, separator="\t")
        print(f"Histone analysis complete. Heatmap saved to {destination}", file=sys.stderr)
